...
//   занимает); от прыжка при схлопывании скелетона страхует возврат
...
export function useInfiniteSentinel(
  ...
  useEffect(() => {
    ...
    // Скелетон живёт в потоке только во время догрузки, сентинел места
    // не занимает — в норме сдвига нет; страховка — вернуть якорь при
    // микро-сдвиге от схлопывания, но не трогать осознанный скролл
    // пользователя (дельта больше допуска).
