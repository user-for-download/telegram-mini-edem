...
try {
  ...
  await runStep("create trip: UI публикует поездку с уникальной ценой", async () => {
    ...
    // DBG: что произошло после клика — навигация, ошибка валидации, ответы сети.
    await page.waitForTimeout(4000);
    console.log("DBG-URL:", page.url());
    const dbgText = await page.evaluate(() => document.body.innerText.slice(0, 1200));
    console.log("DBG-BODY:", JSON.stringify(dbgText));
